...
function RowSkeleton() {
  return (
    <div className="grid grid-cols-[minmax(0,1.6fr)_100px_100px_130px_120px] items-center gap-4 border-t border-border py-3">
...
function TabelaHeader() {
  return (
    <div className="grid grid-cols-[minmax(0,1.6fr)_100px_100px_130px_120px] items-center gap-4 border-b border-border pb-2.5">
      {["JANELA", "PROCESSOS", "INTIMAÇÕES", "QUANDO", "ESTADO"].map((col) => (
        <span
          key={col}
          className="text-[10.5px] font-medium tracking-[0.08em] uppercase text-muted-foreground"
        >
          {col}
        </span>
      ))}
...
function RunRow({
  ...
  return (
    <div
      ...
      className="grid cursor-pointer grid-cols-[minmax(0,1.6fr)_100px_100px_130px_120px] items-center gap-4 rounded-sm border-t border-border py-3 text-[13px] transition-colors hover:bg-muted/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:ring-inset"
    >
  ...
  return (
    <div className="mt-7 flex max-w-5xl flex-col gap-6">
      ...
      {!isPending && !isError && data && (
        <div className="rounded-xl bg-card p-5 ring-hairline">
          <TabelaHeader />
